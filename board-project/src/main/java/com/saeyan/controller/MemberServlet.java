 ...
 * 
 * 역할:
 * - 회원 관련 모든 요청을 받아서 처리하는 중앙 집중식 컨트롤러
 ...
 * 2. 인코딩 설정 (UTF-8)
 * 3. command 파라미터 추출 (기본값: "login_form")
 ...
 * 
 * URL 매핑: /MemberServlet (web.xml에서 설정)
 * 
 * web-study-11 패턴 적용
 */
public class MemberServlet extends HttpServlet {
     ...
     * 2. command 파라미터 추출 및 기본값 설정 ("login_form")
    ...
    private void doProcess(HttpServletRequest request, HttpServletResponse response)
        ...
        if (command == null || command.isEmpty()) {
            command = "login_form"; // 기본값: 로그인 폼
        }
        System.out.println("MemberServlet.doProcess command: " + command);
        ...
        try {
            ...
            System.out.println("Action execute path: " + path);
